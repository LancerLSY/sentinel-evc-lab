...
def make_scene(seed: int = 0) -> Scene:
    ...
    return Scene(
        ...
        obstacles=(Sphere(center=(0.40, 0.0, 0.30), radius=0.05),),
        ws_lo=(0.0, -0.40, 0.0),
...
def perturb(p1: Plan, magnitude: float = 0.003, seed: int = 0, plan_id: str = "NEAR"):
    ...
    for k in p1.points[1:-1]:
        knots.append((k[0], k[1] + rng.uniform(-magnitude, magnitude),
                      k[2] + rng.uniform(-magnitude, magnitude)))
