...
数值参考实现，不需要 GPU、机械臂或模型权重；完整五分钟复现入口待批次 04 验收。

> **本轮实施状态：批次 03 已完成本地验收。** Executor、固定场景、七类故障编排
> 和离线报告已经通过本地回归；完整同流、CLI、独立 1000 案例门禁及本轮样例产物
> 仍等待批次 04 验收。
...
| 证据哈希链与签名 | bundle/verify 已验收；完整同流待批次 04 | 只证明记录完整性，不证明传感器诚实 |
...
批次 04 将把三条判定路径接入同一个 `demo` 事件流和签名包。

---

## 目标运行接口（批次 04 验收前不作为当前复现入口）
...
`sample_run/` 和 [RESULTS.md](RESULTS.md) 是仓库已有的历史样例与记录，尚未按本轮
冻结合同和 G0–G4 重新生成，不能作为当前门禁通过证据。批次 04 只有在 G0–G4
真实通过后才会更新它们。
