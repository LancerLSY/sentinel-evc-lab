...
@pytest.mark.parametrize(
    ("group", "expected"),
    [("runtime", {"numpy", "cryptography"}), ("test", {"pytest"})],
)
def test_dependency_groups_contain_only_approved_packages(group, expected):
    project = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))["project"]
    requirements = (
        project["dependencies"]
        if group == "runtime"
        else project["optional-dependencies"]["test"]
    )
    names = {re.match(r"[A-Za-z0-9_.-]+", item).group().lower() for item in requirements}
    assert names == expected


@pytest.mark.parametrize("capability", ["真实 VLA", "WorldGuard", "真机"])
def test_readme_marks_out_of_scope_capabilities_as_unimplemented(capability):
...
@pytest.mark.parametrize("pattern", ["*.pem", "*.key", "runs/", "SESSION_HANDOFF.md"])
def test_gitignore_covers_secrets_runs_and_local_handoff(pattern):
    rules = {
